...
test.describe('Scenario C: mixed answers', () => {

  test('s1_vars 错-错-对-错-错', async ({ page }) => {
    ...
    const correctPattern = [false, false, true, false, false]  // 错-错-对-错-错
    ...
    for (const [idx, q] of fixture.entries()) {
      const shouldBeCorrect = correctPattern[idx]

      await expect(page.locator('[data-testid="btn-submit"]')).toBeVisible({ timeout: 15000 })
      await page.waitForTimeout(300)

      if (q.type === 'choice' || q.type === 'output_predict') {
        const targetIdx = shouldBeCorrect ? q.correctIndex : ((q.correctIndex ?? 0) + 1) % 4
        const opt = page.locator(`[data-testid="choice-option-${targetIdx}"]`)
      ...
      results.push({ id: q.id, correct: success })
    ...
    console.log(`总题数: ${totalText}`)

    mkdirSync(RESULTS_DIR, { recursive: true })
    writeFileSync(path.join(RESULTS_DIR, 'scenario_C.json'), JSON.stringify({ results, totalQuestions: totalText }, null, 2))
